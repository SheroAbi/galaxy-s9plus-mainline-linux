...
| `bcm4361B2_semco.hcd` | BCM4361 **Bluetooth** patchram | loaded on the device by `hciattach ... bcm43xx` (unit `device/base/etc/systemd/system/hciattach-bcm4361.service`); the image build installs it under `/lib/firmware/brcm/` |

The same three Wi-Fi files are also staged for the rootfs under
`device/base/lib/firmware/brcm/`, already under their final names, for
